...
void FPSCamera::Update(double dt)
{
	...
	static const float CAMERA_ROTATION_SPEED = 120.f;
	...
	{
		Vector3 view = (target - position).Normalized();
		float yaw = (float)(CAMERA_ROTATION_SPEED * (float)dt);
		Mtx44 rotation;
	...
	{
		float pitch = (float)(CAMERA_ROTATION_SPEED * (float)dt);
		Vector3 view = (target - position).Normalized();
		Vector3 right = view.Cross(up);
		right.y = 0;
		right.Normalize();
	...
	{
		Vector3 view = (target - position).Normalized();
		float yaw = (float)(-CAMERA_ROTATION_SPEED * camera_yaw * (float)dt);
		Mtx44 rotation;
		rotation.SetToRotation(yaw, 0, 1, 0);
		view = rotation * view;
		target = position + view;
	...
	{
		float pitch = (float)(-CAMERA_ROTATION_SPEED * camera_pitch * (float)dt);
		Vector3 view = (target - position).Normalized();
		Vector3 right = view.Cross(up);
		right.y = 0;
		right.Normalize();
